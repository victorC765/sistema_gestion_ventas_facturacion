...
-- -----------------------------------------------------
-- Table `gestion-ventas`.`serial`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `gestion-ventas`.`serial` ;

CREATE TABLE IF NOT EXISTS `gestion-ventas`.`serial` (
  `idserial` INT NOT NULL AUTO_INCREMENT,
  `serial` VARCHAR(45) NULL,
  `productos_id-productos` INT NOT NULL,
  PRIMARY KEY (`idserial`),
  UNIQUE INDEX `serial_UNIQUE` (`serial` ASC) ,
  INDEX `fk_serial_productos1_idx` (`productos_id-productos` ASC) ,
  CONSTRAINT `fk_serial_productos1`
    FOREIGN KEY (`productos_id-productos`)
    REFERENCES `gestion-ventas`.`productos` (`id-productos`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
